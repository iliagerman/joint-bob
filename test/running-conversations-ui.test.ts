...
test("running conversations are reachable from projects and the conversation list", async () => {
  ...
  assert.equal((html.match(/data-running-conversations-open/g) || []).length, 3);
  ...
  // The phone header has no room for it, so the chat toolbar carries the third copy.
  assert.match(html, /data-testid="chat-running-conversations-open-button"/);
});
...
test("the running conversations dialog refreshes and opens verified live sessions", async () => {
  ...
  assert.match(worker, /joint-bob-v216/);
