...
async function deleteSession(baseUrl: string, headers: Record<string, string>, projectId: string, sessionPath: string): Promise<Response> {
...
test("session deletion rejects paths not discovered for the selected project", async () => {
  ...
  try {
    ...
    const sessionFile = path.join(claudeProjectPath, "session.jsonl");
    const linkedSession = path.join(claudeProjectPath, "linked-session.jsonl");
    await Promise.all([
    ...
    const discovered = (await listed.json() as { sessions: Array<{ path: string }> }).sessions.find((session) => session.path === `claude:${sessionFile}`);
    assert.ok(discovered);

    for (const candidate of [unrelatedFile, sessionDirectory, linkedSession]) {
      const response = await deleteSession(node.baseUrl, headers, firstProjectId, candidate);
      assert.equal(response.status, 404);
    }
    assert.equal((await deleteSession(node.baseUrl, headers, firstProjectId, `claude:${linkedSession}`)).status, 400);
    assert.equal((await deleteSession(node.baseUrl, headers, firstProjectId, "new")).status, 400);
    assert.equal((await deleteSession(node.baseUrl, headers, firstProjectId, "claude:new")).status, 400);
    assert.equal((await deleteSession(node.baseUrl, headers, secondProjectId, discovered.path)).status, 404);
    ...
    assert.equal((await deleteSession(node.baseUrl, headers, firstProjectId, discovered.path)).status, 204);
