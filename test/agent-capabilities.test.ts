...
test("shared capabilities reach Claude, Pi, and Kiro channels with one logical task scope", async () => {
  ...
  try {
    ...
    try {
      ...
      assert.deepEqual(piFlags, { taskCli: true, taskSocket: true, taskToken: true, browser: true, fixture: "yes" });
      ...
      assert.deepEqual(await client.requestSupervisor(control.socketPath, taskToken, { action: "list" }), []);
    } finally {
