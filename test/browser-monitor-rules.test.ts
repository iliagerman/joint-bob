...
import { browserMonitorRuleCandidates, browserMonitorRuleInputSchema, type BrowserMonitorRuleInput, type BrowserMonitorRuleRecord } from "../src/browser-monitor-rules.js";
...
function event(overrides: Partial<MonitorEvent> = {}): MonitorEvent { return { id: "event", monitorId: "monitor", externalId: "external", targetId: "Target", targetLabel: "Inbox", senderId: "Sender", direction: "incoming", kind: "message.received", text: "Hello World", occurredAt: 1, identity: "stable", observedAt: 100, processed: false, ...overrides }; }
function record(id: string, overrides: Partial<BrowserMonitorRuleRecord> = {}): BrowserMonitorRuleRecord { return { id, monitorId: "monitor", version: 1, input: rule(), enabled: true, createdAt: 1, updatedAt: 1, ...overrides }; }
...
test("rule lifecycle is paused, version-fenced, isolated, and deletion is explicit", () => {
  const x = store(); try {
    ...
    const created = x.store.createRule(monitor.id, rule(), 2); assert.equal(created.version, 1); assert.equal(created.enabled, false);
    assert.throws(() => x.store.getRule(other.id, created.id), /not found/);
    const changed = x.store.updateRule(monitor.id, created.id, 1, rule({ name: "Changed" }), 3); assert.equal(changed.version, 2); assert.equal(changed.enabled, false); assert.equal(changed.monitorId, monitor.id); assert.equal(changed.id, created.id);
    ...
    const enabled = x.store.setRuleEnabled(monitor.id, created.id, 2, true, 4); assert.equal(enabled.version, 3); assert.throws(() => x.store.deleteRule(monitor.id, created.id, 3), /Pause/);
