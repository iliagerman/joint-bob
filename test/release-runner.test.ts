...
function spawnRunner(root: string): Promise<Omit<RunnerResult, "bootstrapInvoked">> {
  return new Promise((resolve, reject) => {
    const child = spawn("/bin/bash", [path.join(root, "scripts/run-node.sh")], {
      cwd: root,
      env: { ...process.env, HOME: root, PATH: `${path.join(root, "bin")}:${process.env.PATH}`, RUNNER_BOOTSTRAP_INVOKED: path.join(root, "bootstrap-invoked") },
    });
    let stdout = "";
    let stderr = "";
...
async function runRunner(releaseMetadata?: string): Promise<RunnerResult> {
  ...
  try {
    ...
    return { ...await spawnRunner(root), bootstrapInvoked: existsSync(path.join(root, "bootstrap-invoked")) };
  } finally {
...
test("runner exports release metadata or development", async () => {
  ...
  assert.equal(valid.bootstrapInvoked, true);
  assert.equal(source.code, 0);
  assert.equal(source.stdout, "development\n");
  assert.equal(source.bootstrapInvoked, true);
});

test("runner rejects invalid release metadata before bootstrap", async () => {
  const release = "0123456789abcdef0123456789abcdef01234567";
  for (const metadata of ["branch=main\n", `commit=${release}\ncommit=${release}\n`, "commit=not-a-commit\n"]) {
    ...
    assert.equal(result.bootstrapInvoked, false);
  }
});
