...
test("routing configurations live under Classifiers, save mappings, and keep unavailable saved models", { timeout: 240_000 }, async (t) => {
  ...
  const harnessSelect = page.getByTestId("routing-harness-select");
  await harnessSelect.waitFor();
  const harnessIds = await page.locator("#routingConfigHarnessGrids [data-routing-harness]").evaluateAll((elements) => elements.map((element) => element.getAttribute("data-routing-harness")));
  assert.deepEqual(await harnessSelect.locator("option").evaluateAll((options) => options.map((option) => option.value)), harnessIds, "the harness picker comes from the available routing harnesses");
  assert.ok(harnessIds.includes("kiro"), "Kiro is available in the dynamic harness picker");
  await harnessSelect.selectOption("kiro");
  assert.equal(await page.locator('#routingConfigHarnessGrids [data-routing-harness]:visible').count(), 1, "only the selected harness mapping is shown");
  const selectedHarness = page.locator('#routingConfigHarnessGrids [data-routing-harness="kiro"]:visible');
  assert.equal(await selectedHarness.count(), 1, "the selected harness owns the visible priorities");
  await page.setViewportSize({ width: 390, height: 844 });
  await page.evaluate(() => new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))));
  const mobileWidth = await selectedHarness.evaluate((element) => ({ client: element.clientWidth, scroll: element.scrollWidth }));
  assert.ok(mobileWidth.scroll <= mobileWidth.client + 1, `the selected harness must not overflow the mobile editor: ${JSON.stringify(mobileWidth)}`);
