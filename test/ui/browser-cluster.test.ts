...
test("conversation browser runs independently of its agent node and stays pinned when defaults change", { timeout: 180000 }, async (t) => {
  ...
  try {
    ...
    const requested=await command({action:'requestLogin',expectedOrigin:fixtureOrigin,readySelector:'#signed-in',loginSelector:'#handoff-login',label:'Fixture login'}) as {result:BrowserSessionView};
    assert.equal(requested.result.loginRequest?.expectedOrigin,fixtureOrigin,'Native login handoff must persist on browser owner');
    const loginRequestId=requested.result.loginRequest!.id;
    const authoritative=await api<{session:BrowserSessionView}>(a,auth,'GET',`/browser/sessions/${session.id}?nodeId=${b.nodeId}`);
    assert.equal(authoritative.status,200,JSON.stringify(authoritative.body));assert.equal(authoritative.body.session.loginRequest?.id,loginRequestId);
