...
test("pending browser login popup preserves ownership through failure, dismissal, reopen, and stale responses", { timeout: 180_000 }, async (t) => {
  ...
  let identity: any, verified = false;
  ...
  const session: any = {
    ...
    loginRequest: { id: requestId, expectedOrigin: "https://accounts.example.test", readySelector: "[data-authenticated]", loginSelector: "input", label: "Synthetic login" },
  };
  const snapshot = () => ({ ...session, ...identity });
  ...
  await page.route("**/api/browser/**", async route => {
    ...
    if (url.pathname === "/api/browser/sessions" && request.method() === "GET") {
      if (identity) { assert.equal(url.searchParams.get("projectId"), identity.projectId); assert.equal(url.searchParams.get("engine"), identity.engine); }
      discoveries.push(url);
      result = { sessions: identity && url.searchParams.get("conversationId") === identity.conversationId ? [snapshot()] : [] };
  ...
  try {
    ...
    identity = await page.evaluate(async () => { const { state } = await import("/app/state.js"); return { projectId: state.activeProjectId, engine: state.engine, conversationId: state.activeConversationId || state.activeSessionId, appNodeId: state.conversationLock?.nodeId || state.activeNodeId }; });
    ...
    assert.deepEqual(commands.filter(command => command.action === "takeControl"), [{ action: "takeControl", loginRequestId: requestId }]);
    broadcast(); broadcast();
    await dialog.getByTestId("browser-control-status").filter({ hasText: "Human control" }).waitFor();
    assert.equal(commands.filter(command => command.action === "takeControl").length, 1, "repeated pending state must not retake control");
    ...
    assert.ok(discoveries.every(url => url.searchParams.get("conversationId") === identity.conversationId));
