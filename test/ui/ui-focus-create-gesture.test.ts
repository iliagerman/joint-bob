...
test("mobile search plus and C/N gestures open project-selectable creation without hijacking scrolling", { timeout: 180_000 }, async t => {
  ...
  assert.equal(await page.getByTestId("new-session-project-select").locator("option:checked").textContent(), "Internal Assistant");
  ...
  await page.getByTestId("new-session-name-dialog").waitFor();
  assert.equal(await page.getByTestId("new-session-project-select").locator("option:checked").textContent(), "Internal Assistant", "C defaults to the active project, not a cancelled selection");
  await page.keyboard.press("Escape");
