...
function fixture(t: TestContext) {
  ...
  const makePage = (): MockPage => {
    ...
    const page = Object.assign(new EventEmitter(), {
      url: () => url, title: async () => "", mainFrame: () => frame, visible, calls,
      visibilityGate: null as MockPage["visibilityGate"], evaluateGate: null as MockPage["evaluateGate"],
      ...
      async evaluate(_expression: string) { calls.evaluate++; if (page.evaluateGate) { page.evaluateGate.entered.resolve(); await page.evaluateGate.release.promise; } return true; },
