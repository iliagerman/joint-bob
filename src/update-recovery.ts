...
async function recoveryDatabase(): Promise<DatabaseSync> {
  ...
  databasePromise = (async () => {
    ...
    const columns = db.prepare("PRAGMA table_info(update_recoveries)").all() as unknown as Array<{ name: string }>;
...
const recoverySettingsSchema = z.object({
  provider: z.string().min(1), modelId: z.string().min(1), reasoning: z.string(), enabledTools: z.array(z.string()).optional(),
}).strict();
interface RecoveryRow { id: string; kind: UpdateRecoveryKind; engine: UpdateRecoveryEngine; project_id: string; cwd: string; session_id: string; session_path: string; task_id: string | null; phase: TaskPhase | null; queued_prompts: string; model: string | null; effort: string | null; settings: string | null; created_at: string; }
export async function listPendingUpdateRecoveries(): Promise<UpdateRecoveryRecord[]> {
  ...
  return rows.map((row) => ({ id: row.id, kind: row.kind, engine: row.engine, projectId: row.project_id, cwd: row.cwd, sessionId: row.session_id, sessionPath: row.session_path, taskId: row.task_id, phase: row.phase, queuedPrompts: JSON.parse(row.queued_prompts) as string[], model: row.model, effort: row.effort, ...(row.settings ? { settings: recoverySettingsSchema.parse(JSON.parse(row.settings)) } : {}), createdAt: row.created_at }));
}
