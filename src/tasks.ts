...
import type { TaskAttachment, TaskEngine, TaskExecutionState, TaskPhaseConfig, TaskRecord, TaskStatus } from "./types.js";
...
export async function createTask(projectId: string, projectPath: string, title: string, description: string, status: TaskStatus, engine: TaskEngine, planMode: boolean, reviewMode: boolean, phaseConfig: TaskRecord["phaseConfig"]): Promise<TaskRecord> {
  ...
  const task: TaskRecord = { id: taskId, title, description, attachments: [], status, engine, planMode, reviewMode, phaseConfig, sessionPath: null, worktreePath: workspacePath, worktreeBranch: null, mergedAt: null, mergeState: "none", conflictCount: 0, mergeWarning: null, mergeTx: null, mergeDigests: baselineDigest ? { baseline: baselineDigest } : null, runKind: null, currentNodeId: node.id, leaseOwnerNodeId: null, leaseExpiresAt: null, executionState: "idle", handoffContext: null, originNodeId: node.id, createdAt: now, updatedAt: now };
...
export function sameTaskSnapshot(left: TaskRecord, right: TaskRecord): boolean {
  ...
  const canonical = (task: TaskRecord): string => JSON.stringify([
    task.id, task.title, task.description, task.attachments ?? [], task.status, task.engine, task.planMode, task.reviewMode, task.phaseConfig,
