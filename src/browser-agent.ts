...
Start explicitly with start [url] [--profile ID | --name LABEL] [--node ID]. Named profiles preserve native browser login data automatically; the user signs in through Take control, without a separate Secrets account. A profile can hold several websites; use separate profiles for different accounts on the same website. Profiles are node-owned entities pinned to their machine; they never replicate or fail over. Access is decided by grants: a new profile is granted to exactly the conversation that created it, and the user can widen it in the viewer's Access panel to a project, more conversations, or every conversation on that node. Agents list and start only profiles granted to their conversation — including cold profiles that have not run yet — and a bare start --profile ID routes to the profile's owning machine with no fallback. One profile runs in only one conversation at a time; end it there first. Never guess which account to use or supply another project/session identity. Use status, tabs, profiles, navigate URL, snapshot, click SELECTOR, fill SELECTOR TEXT, evaluate EXPRESSION, close, or save-login LABEL. save-login names the current profile; it no longer clones login snapshots. Use command '{"action":"..."}' for other browser commands. Run interactive test assertions through evaluate and read the returned results; do not claim tests passed without checking them. Manual takeover pauses agent commands. Wait for the user to resume agent control; do not override takeover. A revoked grant pauses this conversation's automation at once — its status shows the session as access-revoked without live page details, and closing stays available; ask the user to restore the grant in the viewer's Access panel.
