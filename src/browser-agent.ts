...
Browser machine selection for new sessions is: explicit start --node ID > conversation override > Settings default. Existing sessions and profiles remain pinned to their browser machine. Changing defaults or moving the agent never migrates browser login data. Use status to discover machine IDs and attached sessions. Never fall back to another machine when the chosen one is unavailable. Joint Bob supports Ubuntu and macOS and never installs browsers automatically.
...
Login handoff: Gmail authentication redirects automatically open a sign-in popup for the displayed conversation. For other sites, issue command JSON with action requestLogin, the exact expectedOrigin, a specific visible authenticated readySelector, and optional loginSelector and label. Always pass an explicit --profile when multiple accounts run. A login request durably pauses browser commands: tell the user to sign in through the popup using the existing viewer and choose Done. Dismissing the popup or restarting leaves the browser paused; do not retry blocked commands or call completeLogin as the agent. Only the human Done action verifies the positive marker and resumes browser actions. This verification is not a bypass of website security or a guarantee that authentication succeeded.

Service restarts reopen persistent profiles to their sites' origins with fresh page IDs and preserve human control. Reinspect the current state before acting. Never replay an interrupted send, form submission, upload, or trade. Explicit close or closing the last tab ends only the chosen browser and disables its automatic reopening; its profile remains saved. Sites can expire logins or require MFA again. For WhatsApp, the user links WhatsApp Web from their phone's Linked devices screen. Read and summarize requested chats; send replies only when authorized by the user. Bank access is read-only: payments, transfers and real-money trades stay manual. Paper trading requires an explicit request and verification that the site is in paper mode, not merely a profile labelled paper.
