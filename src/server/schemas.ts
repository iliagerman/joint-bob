...
export const secretAccountSchema = z.object({ label: z.string().trim().min(1).max(64).refine((value) => !/[\x00-\x1f\x7f]/.test(value), "Secret account label cannot contain control characters"), provider: z.enum(["aws", "google", "github", "custom", "website"]), replicate: z.boolean().optional(), websiteOrigin: websiteOriginSchema.nullable().optional(), projectId: z.string().trim().min(1).max(300).optional(), variables: z.array(secretVariableSchema).min(1).max(20) }).strict().refine((value) => !(value.projectId && value.replicate), "Project-scoped secret accounts cannot replicate");
