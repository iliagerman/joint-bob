...
const configuration: HarnessConfiguration = { defaults: (home) => ({ executable: detectExecutable("kiro-cli"), configPath: path.join(home, ".kiro"), sessionPath: path.join(home, ".kiro/sessions") }), fixedProvider: "kiro", thinkingLevels: ["low", "medium", "high", "xhigh", "max"], restartFields: ["executable", "configPath"] };
