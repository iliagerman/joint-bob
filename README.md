...
Browsers remain headless by default. On terminal-only Linux, opt into headed Chrome on a private virtual display by setting `JOINT_BOB_BROWSER_MODE=virtual` in the node environment. Install the platform packages manually on Ubuntu with administrator approval, for example `sudo apt install xvfb xauth`; Joint Bob does not install them. The installed `scripts/run-node.sh` starts `xvfb-run` only in virtual mode, with private Xauthority and TCP listening disabled. It does not silently fall back if Linux, Xvfb, xauth, or the display is unavailable. A direct development server must instead be started manually through `xvfb-run` with `DISPLAY` available.

Virtual mode uses the same native profiles, viewer, and Playwright automation as headless mode; Chrome remains automated and there is no guarantee that Google or another site will accept sign-in. Changing mode requires a full native-service restart, which interrupts managed tasks while the supervisor and app restart. Wait for active jobs to finish first; browser sessions can reconnect through normal restart recovery. Virtual mode is Linux-only; macOS and other platforms must use the unchanged headless mode.
...
When a running browser requests login, Joint Bob automatically opens its existing authenticated viewer for that conversation and profile. Gmail authentication redirects can request this automatically; agents explicitly request handoff for other sites. Choose **Done** only after signing in: Joint Bob checks the requested positive page marker, which is not a guarantee from Google or a bypass of site security. Dismissing the popup or restarting leaves browser actions paused; reopen **Browser** to finish. This uses the existing viewer and requires no additional dependency installation.
