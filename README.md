...
When any running browser shows a common visible password or username-first form without matching origin-bound Secrets, or a visible login challenge, Joint Bob opens a persistent sign-in popup for that browser's conversation and profile. This works across projects and background conversations, including Pi, Claude, and Kiro sessions and browsers displayed in the board or canvas; no conversation must be actively selected. Matching ordinary Secrets remain automatic, and Secrets are not required for manual sign-in.

Sign in through the popup, then choose **Done**. For an automatic handoff, Done checks that the visible login or challenge is gone and that a nonempty ready page remains at the detected or original origin; it then hands control back on the human's confirmation. This is not a guarantee of authentication or a bypass of site security, so the agent reinspects the authenticated state. An explicit `requestLogin` can provide a site-specific positive marker. Dismissing the popup or restarting never resumes automation; open the requesting conversation's **Browser** panel to finish. Custom controls and unrecognized challenges require the agent to request an explicit handoff, so arbitrary sites are not guaranteed to work.
